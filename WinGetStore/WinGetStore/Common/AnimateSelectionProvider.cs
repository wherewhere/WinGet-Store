...
namespace WinGetStore.Common
{
    public class AnimateSelectionProvider : DependencyObject
    {
        ...
        private static readonly Vector2 c_frame2point1 = new(0.1f, 0.9f);
        private static readonly Vector2 c_frame2point2 = new(0.2f, 1f);
        ...
        private UIElement FindSelectionIndicator(Pivot pivot, object item)
        {
            if (item != null && pivot != null)
            {
                ...
                if (index != -1)
                {
                    if (pivot.FindDescendants().OfType<PivotHeaderPanel>().Where(x => x.Children.Count > 0).FirstOrDefault()?.Children[index] is UIElement container)
                    {
                        if (container.FindDescendant(IndicatorName) is UIElement indicator)
                        {
                            return indicator;
