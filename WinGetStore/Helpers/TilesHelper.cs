...
namespace WinGetStore.Helpers
{
    public static class TilesHelper
    {
        ...
        public static void UpdateTile(this XmlDocument tileContent)
        {
            ...
            {
                SettingsHelper.LoggerFactory.CreateLogger(typeof(TilesHelper)).LogError(ex, "Failed to update tile. {message} (0x{hResult:X})", ex.GetMessage(), ex.HResult);
            }
        }
        ...
        public static void UpdateTiles(this IEnumerable<XmlDocument> tileContents)
        {
            ...
            {
                SettingsHelper.LoggerFactory.CreateLogger(typeof(TilesHelper)).LogError(ex, "Failed to update tiles. {message} (0x{hResult:X})", ex.GetMessage(), ex.HResult);
            }
        }
        ...
        public static async Task UpdateAvailablePackageAsync()
        {
            ...
            {
                SettingsHelper.LoggerFactory.CreateLogger(typeof(TilesHelper)).LogError(ex, "Failed to update available package tile. {message} (0x{hResult:X})", ex.GetMessage(), ex.HResult);
                return;
            }
        }
        ...
        private static async Task<PackageCatalog> CreatePackageCatalogAsync()
        {
            ...
            {
                SettingsHelper.LoggerFactory.CreateLogger(typeof(TilesHelper)).LogError(ex, "Failed to create package catalog. {message} (0x{hResult:X})", ex.GetMessage(), ex.HResult);
                return null;
            }
        }
        ...
        private static async Task<FindPackagesResult> TryFindPackageInCatalogAsync(PackageCatalog catalog)
        {
            ...
            {
                SettingsHelper.LoggerFactory.CreateLogger(typeof(TilesHelper)).LogError(ex, "Failed to find packages. {message} (0x{hResult:X})", ex.GetMessage(), ex.HResult);
                return null;
            }
        }
    }
